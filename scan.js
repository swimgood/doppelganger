...
export default async function handler(req, res) {
  ...
  let posts;
  try {
    posts = await fetchRecentPosts(handle);
  } catch (err) {
    console.error('[scan] fetchRecentPosts threw:', err);
    posts = null;
  }

  if (!posts) {
    console.log(`[scan] @${handle}: live fetch returned nothing (fetch_failed)`);
    res.status(200).json(generateFallback(handle, nonce, 'fetch_failed'));
    return;
  }
  if (posts.length < 3) {
    console.log(`[scan] @${handle}: only ${posts.length} posts returned (too_few_posts)`);
    res.status(200).json(generateFallback(handle, nonce, 'too_few_posts'));
    return;
  }
  ...
  if (candidates.length === 0) {
    console.log(`[scan] @${handle}: candidate pool is empty (no_candidates) — run scripts/build-candidates.mjs`);
    res.status(200).json(generateFallback(handle, nonce, 'no_candidates'));
    return;
  }

  const match = findBestMatch(features, candidates);
  if (!match) {
    console.log(`[scan] @${handle}: findBestMatch returned nothing (no_match)`);
    res.status(200).json(generateFallback(handle, nonce, 'no_match'));
    return;
  }
